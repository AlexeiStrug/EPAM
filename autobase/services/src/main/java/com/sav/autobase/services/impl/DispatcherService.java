...
public class DispatcherService implements IDispatcherService {
	...
	public Request getRequest() throws DAOException {
		try {
			...
			if (request.getDispatcher() == null) {
				return requestDao.updateClientRequest(request);
			} else
	...
	public void modifyRequest(Request request) throws DAOException {
		if (request != null) {
			...
			LOGGER.info(
					"Updated request Request.id={}. start_date={}. end_date={}. place_id={}. count_of_passenger={}. dispatcher={}. processed={} ",
					request.getId(), request.getStartDate(), request.getEndDate(), request.getPlace(),
					request.getCountOfPassenger(), request.getDispatcher(), request.getProcessed().name());
		} else
